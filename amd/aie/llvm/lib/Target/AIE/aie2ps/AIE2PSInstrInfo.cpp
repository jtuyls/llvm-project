...
AIE2PSInstrInfo::AIE2PSInstrInfo(const AIEBaseSubtarget &STI, const TargetRegisterInfo &TRI)
    : AIE2PSGenInstrInfo(STI, TRI, AIE2PS::ADJCALLSTACKUP, AIE2PS::ADJCALLSTACKDOWN) {
...
void AIE2PSInstrInfo::loadRegFromStackSlot(
  ...
  auto bounceViaRegClass = [&](const TargetRegisterClass *BounceRC) {
    MachineRegisterInfo &MRI = MBB.getParent()->getRegInfo();
    Register TmpReg = MRI.createVirtualRegister(BounceRC);
    loadRegFromStackSlot(MBB, I, TmpReg, FI, BounceRC, VReg, /*SubReg=*/0, Flags);
