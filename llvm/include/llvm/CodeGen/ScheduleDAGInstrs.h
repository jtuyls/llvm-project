...
namespace llvm {
  ...
  public:
    ...
    /// amd/aie/ port: record the debug instructions of the current region.
    /// Factored out of buildEdges() because buildEdges() iterates SUnits (which
    /// debug instructions never have), not the MI range.
