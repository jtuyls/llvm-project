...
TGInstrLayout::TGInstrLayout(const CodeGenInstruction *const CGI,
  ...
  // amd/aie/ port: getSuperClasses() now returns ArrayRef<const Record *>.
...
void TGInstrLayout::resolveIsSlot() {
  ...
  if (IsComposite) {
    ...
    for (auto *Field : leaves()) {
      ...
      // amd/aie/ port: hasOperandNamed(name, &idx) -> findOperandNamed -> optional.
      if (auto OpIdxOpt = CGI->Operands.findOperandNamed(Field->Label)) {
...
void TGInstrLayout::resolveMCOperandNumber() {
  ...
  for (auto *Field : leaves()) {
    ...
    if (auto OpIdxOpt = CGI->Operands.findOperandNamed(Field->Label)) {
      ...
      unsigned OpIdx = CGI->Operands[*OpIdxOpt].MIOperandNo;
...
void TGFieldLayout::resolveFieldsDefInHierarchy(
  ...
  // amd/aie/ port: getSuperClasses() now returns ArrayRef<const Record *>.
  ArrayRef<const Record *> Hierarchy = BaseRecord->getSuperClasses();
  const BitsInit *BI = nullptr;
  ...
  const Record *CurrentLevelRecord = Hierarchy[HierarchyLevel]; // amd/aie/ port
