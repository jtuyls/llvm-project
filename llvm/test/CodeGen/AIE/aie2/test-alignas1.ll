...
define <16 x i16> @_Z12test_alignasv() {
  ...
  ; CHECK-NEXT:   [[LOAD:%[0-9]+]]:_(<16 x s16>) = G_LOAD [[COPY1]](p0) :: (load (<16 x s16>) from %ir.arrayidx, align 256)
  ; CHECK-NEXT:   G_STORE [[LOAD]](<16 x s16>), [[FRAME_INDEX1]](p0) :: (store (<16 x s16>) into %ir.b, align 1024)
  ; CHECK-NEXT:   [[LOAD1:%[0-9]+]]:_(<16 x s16>) = G_LOAD [[FRAME_INDEX1]](p0) :: (load (<16 x s16>) from %ir.b, align 1024)
...
define i32 @_Z13test_alignas1v() {
  ...
  ; CHECK-NEXT:   [[COPY:%[0-9]+]]:_(p0) = COPY [[FRAME_INDEX]](p0)
  ; CHECK-NEXT:   [[LOAD:%[0-9]+]]:_(s32) = G_LOAD [[COPY]](p0) :: (load (s32) from %ir.n, align 512)
  ; CHECK-NEXT:   G_STORE [[LOAD]](s32), [[FRAME_INDEX1]](p0) :: (store (s32) into %ir.b, align 64)
  ; CHECK-NEXT:   [[LOAD1:%[0-9]+]]:_(s32) = G_LOAD [[GV]](p0) :: (load (s32) from @n9, align 1024)
