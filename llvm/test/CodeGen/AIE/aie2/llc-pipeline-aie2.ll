...
; RUN:   | grep -v 'Verify generated machine code' | FileCheck -match-full-lines -strict-whitespace -check-prefix=AIE-O1 %s
; RUN: llc -O2 -mtriple=aie2 -disable-verify -debug-pass=Structure < %s 2>&1 \
; RUN:   | grep -v 'Verify generated machine code' | FileCheck -match-full-lines -strict-whitespace -check-prefix=AIE-O23 %s
...
; AIE-O1-NEXT:      Constrain partially-allocatable classes
; AIE-O1-NEXT:      Live Interval Analysis
...
; AIE-O23-NEXT:      Constrain partially-allocatable classes
; AIE-O23-NEXT:      Live Interval Analysis
